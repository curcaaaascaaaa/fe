...
const Regulations = () => {
  ...
  function createData(epoch, dao, dev, lodge, bondsBought, bondsRedeemed) {
    var sum = (Number(dao) + Number(dev) + Number(lodge)).toFixed(2);
    return { epoch, dao, dev, lodge, sum, bondsBought, bondsRedeemed };
  }
  useEffect(() => {
    if (frostFinance) {
        ...
        <Table className={classes.table} aria-label="simple table">
          <TableHead>
            <TableRow>
              ...
              <StyledTableCell align="center">Lodge funding</StyledTableCell>
          ...
          <TableBody>
            {rows?.map((row, index) => (
              <StyledTableRow
                ...
                key={row.epoch}
              >
                <StyledTableCell style={{ color: '#092E48' }} align="center" component="th" scope="row">
                  {row.epoch}
                </StyledTableCell>
                <StyledTableCell align="center">{row.lodge}</StyledTableCell>
